...
const Earn = () => {
    ...
    async function fetchTasks() {
      ...
      if (locale == "en") {
        for (const el in data) {
          const transDes = await textTranslate(
            data[el].quest.description,
            "ru",
            "en"
          );
          const transName = await textTranslate(
            data[el].quest.name,
            "ru",
            "en"
          );
  ...
  const openModal = async (type, id, price, value, premium, completed) => {
    if (type === "success-speed") {
      ...
      const textCheck = await textTranslate(
        ...
        <div className="container">
          ...
          <div className="header_bar anim_sjump">
            <div className="content">
              <img
                className="back_img"
                src={balance[`coin${balanceIdImage > 0 ? balanceIdImage : 0}`]}
                alt="imgCoins"
              />
              <p className="coins">
                {userInfo.storage}/{capacityValue} <IconCoin />
              </p>
              <span className="per">
                    ...
                    .map((task) => (
                      ...
                      >
                        <img
                          ...
                          src={`res/quests/${task.quest.imgPath}`}
                          alt="task_picture"
                        />
                        <div className="content">
